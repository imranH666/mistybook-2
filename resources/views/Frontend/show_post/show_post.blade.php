                            ...
                            <div class="row g-1">
                                ...
                                @foreach ($post->rel_to_post_images as $post_image)
                                    <div class="col-lg-{{ $imageCount == 1 ? '12' : '6' }} col-{{ $imageCount == 1 ? '12' : '6' }} col-sm-{{ $imageCount == 1 ? '12' : '6' }} col-md-{{ $imageCount == 1 ? '12' : '6' }}">
                                        <img src="" data-src="{{ asset('upload/posts') }}/{{ $post_image->image_path }}" alt="Loading" class="lazy"/>
                                    </div>
                                @endforeach
                            </div>
                        </div>
                        <div class="post-actions">
                            <div class="like-comment">
                                <div>
                                    ...
                                    <span class="sm-like-count">({{ $post->rel_to_like_count->count() }})</span>
                                </div>
            ...
            const observer = new IntersectionObserver((entries, observer) => {
                entries.forEach(entry => {
                    if (entry.isIntersecting) {
                        ...
                        image.src = image.dataset.src; // আসল ইমেজ লোড
                        image.classList.remove("lazy"); // ক্লাস সরান
                        observer.unobserve(image); // আর পর্যবেক্ষণ করবেন না
                    }
                });
            });
